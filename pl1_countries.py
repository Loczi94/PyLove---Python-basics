...
def not_in_africa():
    for country in countries:
        if not country["region"]=="Africa":
            print(country["name"]["official"])

def currency():
    for country in countries:
        if len(country["currency"])>1:
            print("Kraj: "+str(country["name"]["official"])+"\t\tWaluty: "+str(country["currency"]))

def capital():
    for country in countries:
        if len(country["capital"])>0:
            if country["capital"][0]=="W":
                print("Kraj: " + str(country["name"]["official"]) + "\t\tStolica: " + str(country["capital"]))

def area():
    area=0
    name=""
    for country in countries:
        if int(country["area"])>area:
            area=int(country["area"])
            name=country["name"]["official"]
    print("Kraj: " +name + "\t\tPowierzchnia: " + str(area))

def borders():
    borders = 0
    name = ""
    for country in countries:
        if len(country["borders"])>borders:
            borders=len(country["borders"])
            name = country["name"]["official"]
    print("Kraj: " + name + "\t\tLiczba sąsiadujących państw: " + str(borders))


if __name__ == '__main__':
    print('Wynik działania funkcji borders()')
    borders()
    print('\nWynik działania funkcji area()')
    area()
    print('\nWynik działania funkcji capital()')
    capital()
    print('\nWynik działania funkcji currency()')
    currency()
    print('\nWynik działania funkcji not_in_africa()')
    not_in_africa()
    print('\nWynik działania funkcji northern_america()')
    northern_america()